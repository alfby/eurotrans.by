 ...
 */
	public static function formatCurrencySign($price, $currency, $separator = "")
	{
		...
		return "<span class='tot-summ'>" . $price . "</span>" . $separator . self::getCurrencySign($currency);
	}
/**
 ...
 * @return string
 */
        public static function formatTime2Date($time,$inputFormat = 'Y-m-d H:i:s',$outputFormat = 'Y-m-d')
        {
            $dateObj = DateTime::createFromFormat($inputFormat, $time);
	...
	public static function formatDate($date, $inputFormat, $outputFormat = "Y-m-d")
	{
        ...
        public static function getDateArray($time,$dateFormat,$optionDateFormat){
            //$date['date'] = self::formatTime2Date($time, 'Y-m-d H:i:s',$optionDateFormat);
            $date['current_date'] = $time;
            $date['selected_date'] = self::formatDate($date['current_date'], $optionDateFormat);
            $date['previous_date'] = self::formatDate(date($dateFormat, strtotime($date['selected_date'] . ' -1 day')), $dateFormat, $optionDateFormat);
            $date['next_date'] = self::formatDate(date($dateFormat, strtotime($date['selected_date'] . ' +1 day')), $dateFormat, $optionDateFormat);
	...
	public static function formatTime($time, $inputFormat, $outputFormat = "H:i:s")
	{
            ...
            foreach ($limiters as $limiter)
            {
                if (strpos($inputFormat, $limiter) !== false)
                {
                    ...
                    if (isset($_time[1]) && (strpos($_time[1], " ") !== false))
                    {
