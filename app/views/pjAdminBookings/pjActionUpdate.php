...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionUpdate" method="post" class="form pj-form" id="frmUpdateBooking">
		...
		<div id="tabs">
			...
			<div id="tabs-1">
				<div class="bs-loader-outer">
					...
					<p>
						...
						<span class="inline-block l10">
							<?php __('lblTransfer');?> 
							<input type="checkbox" id="is_transfer" name="is_transfer" <?php echo (int)$tpl['arr']['is_transfer'] == 1 ? 'checked="checked"' : null; ?> />
						</span>
												
						<span class="inline-block l10">
							<select name="transfer_booking_id" id="transfer_booking_id" class="pj-form-field w150 <?php echo (int)$tpl['arr']['is_transfer'] == 1 ? 'required' : null; ?>" <?php echo (int)$tpl['arr']['is_transfer'] == 0 ? 'disabled="disabled"' : null; ?>>
								<option value="">-- <?php __('lblChoose'); ?>--</option>
								<?php
								foreach($tpl['transfer_booking_arr'] as $k => $v)
								{
									?><option value="<?php echo $v['id'];?>"<?php echo $v['id'] == $tpl['arr']['transfer_booking_id'] ? ' selected="selected"' : null; ?>><?php echo pjSanitize::clean($v['uuid']);?></option><?php
								} 
								?>
							</select>
						</span>
					...
					<?php if ((int)$tpl['arr']['is_transfer'] == 1) { ?>
						<p>
							<label class="title">&nbsp;</label>
							<span class="inline-block"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionUpdate&id=<?php echo $tpl['arr']['transfer_booking_id'];?>"><?php __('lblTransferBooking');?></a></span>
						</p>
					<?php } ?>
					<p>
						...
						{ 
							...
							<p>
								...
								<span class="block overflow">
									...
									if (isset($controller->defaultTicketCurrencies[$tpl['arr']['currency']])) {
										$selected_price = $controller->defaultTicketCurrencies[$tpl['arr']['currency']];
									}
									foreach($tpl['ticket_arr'] as $v)
									{
										if($v[$selected_price] != '')
										{
											$price = $v[$selected_price] - ($v[$selected_price] * $v['discount'] / 100);
											?>
											<span class="block b5 overflow">
												...
												<label class="block float_left t5"><?php echo pjUtil::formatCurrencySign( number_format($price, 2), $tpl['arr']['currency']);?></label>
											</span>
											<?php
										}
									} 
			...
			<div id="tabs-2">
				<h2 class="passengerDetailsTitle"><?php __('lblPassengersDetails');?></h2>
				<div class="pjPassengerListsContainer">
