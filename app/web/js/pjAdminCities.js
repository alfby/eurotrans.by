...
(function ($, undefined) {
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
            ...
            $('#av-cities-container input').each(function(){
                cityId = $(this).val();
                $('#av-cities-list option[value='+cityId+']').hide();
            });
            
            //add city page//
            $('#av-cities-container').on('click','.pj-table-icon-delete',function(){
                var localeId = $('.multilang.pj-form-langbar a.pj-form-langbar-item-active').data('index');
                parent = $(this).parent('.av-city-wrapper');
                tgCityId = parent.find('input').val();
                $('#av-cities-list option[value='+tgCityId+'][data-locid$="'+localeId+'"]').show();
                parent.remove();
            ...
            $('#av-cities-list').change(function(){
                var localeId = $('.multilang.pj-form-langbar a.pj-form-langbar-item-active').data('index');
                tgCityId = $(this).val();
                if('' === tgCityId){
                    return false;
                }
                chosenOpts = $('#av-cities-list option[value='+tgCityId+'][data-locid$="'+localeId+'"]');
                tgCityName = chosenOpts.text();
                $('#av-cities-list option[value='+tgCityId+']').hide();
                
                if(0 === $('#av-cities-list option[style$="display:block"]').size()){
                    $('#av-cities-list').hide();
                }
                html = "<span class='av-city-wrapper'>";
                html += "<input type='hidden' name='av_cities[id]["+tgCityId+"]' value='"+tgCityId+"'>";
                html += "<span class='av-cities-name'>";
                html += tgCityName;
                html += "</span>";
                html += "<a href='javascript:void(0)' class='pj-table-icon-delete'></a>";
                html += "</span>";
                
                $('#av-cities-list option[value=""]').attr("selected","selected");//set default option

                $('#av-cities-container').append(html);
            });
            //==add city page==//
	});
})(jQuery_1_8_2);