	...
	BusReservation.prototype = {
		...
		init: function (opts) {
			...
			this.$container.on("click.bs", ".bsSelectorLocale", function (e) {
				...
				return false;
                        }).on("change.bs",".bsTicketExchangeSelect",function(e){
                            agregateSumm = true;
                            var chosenQnt = pjQ.$(this).val();
                            var ticketType = pjQ.$(this).data('ticket-type');
                            pjQ.$('[data-ticket-type="'+ticketType+'"]').each(function(){
                                pjQ.$(this).val(chosenQnt);
                                if(pjQ.$(this).hasClass('bsTicketSelect')){
                                    pjQ.$(this).trigger('change.bs2');
                                }
                                else if(pjQ.$(this).hasClass('bsReturnTicketSelect')){
                                    pjQ.$(this).trigger('change.bs3');
                                }
                            });
                        }).on("change.bs2", ".bsTicketSelect", function (e) {
                            var $this = pjQ.$(this),
                                bus_id = pjQ.$(this).attr('data-bus'),
                                pickup_id = pjQ.$(this).attr('data-pickup_id'),
                                return_id = pjQ.$(this).attr('data-return_id'),
                                strIds = pickup_id + '' + return_id,
                                seat_map = pjQ.$(this).attr('data-set'),
                                total_tickets = 0,
                                seats = '',
                                $seat_label = pjQ.$("#bsSeats_" + self.opts.index + strIds),
                                $selected_bus = pjQ.$("#bs_selected_bus_" + self.opts.index + strIds),
                                selected_bus_id = $selected_bus.val(),
                                $selected_seats_label = pjQ.$('.bsSelectedSeatsLabel_' + self.opts.index + strIds),
                                ...
                                pjQ.$(".bsTicketSelect").prop('disabled',false);
                            ...
                            summ=0;
                            pjQ.$('.pjBsTotalPrice .tot-summ').text(0);
                            pjQ.$.post([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionGetRoundtripPrice&bus_id=", bus_id, "&return_bus_id=", return_bus_id,"&is_return=","F","&pickup_id=", pickup_id, "&return_id=", return_id,"&summ=",summ].join(""), $form.serialize()).done(function (data) {
                                pjQ.$('#bsRoundtripPrice_' + self.opts.index).html(data);
                                if(agregateSumm){
                                    var summRes = parseFloat(pjQ.$('.pjBsTotalPrice .tot-summ').text());
                                    summ = summ+summRes;
                                    pjQ.$('.pjBsTotalPrice .tot-summ').text(summ.toFixed(2));    
                                }
                            ...
                            pjQ.$('#bs_selected_tickets_' + self.opts.index + strIds).val(total_tickets);
                            pjQ.$('#bs_has_map_' + self.opts.index + strIds).val(seat_map);
                            self.onReselect(pickup_id,return_id);
                            if(total_tickets != 1)
                            {
                            ...
                            if(total_tickets == 0)
                            {
                                ...
                                if(seat_map == 'F')
                                {
                                    ...
                                    $selected_seats_label.parent().css('display', 'none');
                                    pjQ.$('#bs_selected_seats_' + self.opts.index + strIds).val(selected_id_arr.join("|"));
                                }
                            }
			}).on("change.bs3", ".bsReturnTicketSelect", function (e) {
                            var $this = pjQ.$(this),
                                pickup_bus_id = null,
                                bus_id = pjQ.$(this).attr('data-bus'),
                                pickup_id = pjQ.$(this).attr('data-pickup_id'),
                                ...
                                total_tickets = 0,
                                seats = '',
                                $seat_label = pjQ.$("#bsReturnSeats_" + self.opts.index + strIds),
                                $selected_bus = pjQ.$("#bs_return_selected_bus_" + self.opts.index + strIds),
                                selected_bus_id = $selected_bus.val(),
                            ...
                            pjQ.$.post([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionGetRoundtripPrice&bus_id=", bus_id, "&return_bus_id=", return_bus_id, "&is_return=","T","&pickup_id=", pickup_id, "&return_id=", return_id,"&summ=",summ].join(""), $form.serialize()).done(function (data) {
                                if(agregateSumm){
                                    var summ = parseFloat(pjQ.$('.pjBsTotalPrice .tot-summ').text());
                                    pjQ.$('#bsRoundtripPrice_' + self.opts.index).html(data);
                                    var summRes = parseFloat(pjQ.$('.pjBsTotalPrice .tot-summ').text());
                                    var totSumm = summ+summRes;
                                    var formatedTotSumm = totSumm.toFixed(2);
                                    pjQ.$('.pjBsTotalPrice .tot-summ').text(formatedTotSumm);
                                }else{
                                    pjQ.$('#bsRoundtripPrice_' + self.opts.index).html(data);
                                }
                            });
                            ...
                            if(selected_bus_id != '')
                            {
                                if(bus_id != selected_bus_id)
                                {
                                    ...
                                    if(seat_map == 'T')
                                    {
                                        ...
                                        self.loadReturnMap(bus_id, pickup_id, return_id);
                                    }else{
                                        $seat_label.parent().parent().css('display', 'none');
                            ...
                            pjQ.$('#bs_return_selected_tickets_' + self.opts.index + strIds).val(total_tickets);
                            pjQ.$('#bs_return_has_map_' + self.opts.index + strIds).val(seat_map);
                            self.onReselect(pickup_id,return_id);
                            ...
                            var seat_id = pjQ.$(this).attr('data-id'),
                                ...
                                mapContainer = pjQ.$(this).closest('.bsMapContainer'),
                                data_pickup_id = mapContainer.data('pickup_id'),
                                data_return_id = mapContainer.data('return_id'),
                                seat_arr = self.getSeatsArray(data_pickup_id,data_return_id),
                                seat_name_arr = self.getSeatsNameArray(data_pickup_id,data_return_id),
                                bus_container = pjQ.$(this).closest('.bsBusContainer'),
                                quantity = parseInt(bus_container.find('.bs_selected_tickets').val(), 10),
                                $selected_seats_label = bus_container.find('.bsSelectedSeatsLabel');
                        	
                                if(quantity > seat_arr.length && pjQ.$.inArray(seat_id,seat_arr) == -1)
				{
                                    pjQ.$(this).addClass('bs-selected');
                                    seat_arr.push(seat_id);
                                    bus_container.find('.bs_selected_seats').val(seat_arr.join("|"));
                                    seat_name_arr.push(seat_name);
                                    $selected_seats_label.html(seat_name_arr.join(", "));
                                    bus_container.find('.bsReSelect').css('display', 'inline-block');
                                    bus_container.find('.bsSeatErrorMsg').css('display', 'none');
				}
                            ...
                            if(quantity > seat_arr.length && pjQ.$.inArray(seat_id,seat_arr) == -1)
                            {
                                ...
                                seat_name_arr.push(seat_name);
                                $selected_seats_label.html(seat_name_arr.join(", "));
                                bus_container.find('.bsReturnReSelect').css('display', 'inline-block');
                                bus_container.find('.bsReturnSeatErrorMsg').css('display', 'none');
                            }
			}).on("click.bs", ".bsReSelect", function (e) {
                            if (e && e.preventDefault) {
                                e.preventDefault();
                            }
                            var pickup_id = pjQ.$(this).data('pickup_id'),
                            return_id = pjQ.$(this).data('return_id');
                            self.onReselect(pickup_id,return_id);
			})
				...
				hashBang("#!/Search");
                            ...
                            pjQ.$('.bs_selected_tickets').each(function() {
                                qnt = pjQ.$(this).val();
                                has_map = pjQ.$(this).attr('data-map');
                                var busContainer = pjQ.$(this).closest('.bsBusContainer');
                                var pickup_id = busContainer.data('pickup_id'),
                                return_id = busContainer.data('return_id'),
                                number_of_tickets = parseInt(qnt,10),
                                strIds = pickup_id.toString()+return_id.toString(),
                                uniqueStr = self.opts.index + strIds;
                                
                                if(('0' == qnt) || ('' == qnt)){
                                    busContainer.find('.bsTicketErrorMsg').css('display','block');
                                    busContainer.find('.pjBsPickupSeatsBody').show();
                                    valid = false;
                                }else{
                                    if (pjQ.$('#bs_return_selected_tickets_' + uniqueStr).length > 0) 
                                    {
                                        var return_number_of_tickets = pjQ.$('#bs_return_selected_tickets_' + uniqueStr).val(),
                                            return_has_map = pjQ.$('#bs_return_has_map_' + uniqueStr).val();
                                        ...
                                        if(return_number_of_tickets == '' || return_number_of_tickets == '0') 
                                        {
                                            ...
                                            if(has_map == 'T' || return_has_map == 'T')
                                            {
                                                var selected_seats = pjQ.$('#bs_return_selected_seats_' + uniqueStr).val();
                                                if(selected_seats == '')
                                                {
                                                    busContainer.find('.bsSeatErrorMsg').html(self.opts.validation.required_seat);
                                                    busContainer.find('.bsSeatErrorMsg').css('display', 'block');
                                                    busContainer.find('.pjBsPickupSeatsBody').show();
                                                    valid = false;
                                                }else{
                                                    ...
                                                    if(return_selected_seats == '')
                                                    {
                                                        ...
                                                        var seat_arr = self.getSeatsArray(pickup_id,return_id);
                                                        if(seat_arr.length < parseInt(number_of_tickets, 10))
                                                        {
                                                            ...
                                                            var return_seat_arr = self.getSeatsArray(pickup_id,return_id);
                                        ...
                                        if(has_map == 'T') {
                                            var selected_seats = busContainer.find('.bs_selected_seats').val(),
                                                seatsErrMsgBlock = busContainer.find('.bsSeatErrorMsg');
                                        
                                            if(selected_seats == '')
                                            {
                                                seatsErrMsgBlock.html(self.opts.validation.required_seat);
                                                seatsErrMsgBlock.css('display', 'block');
                                                valid = false;
                                            }else{
                                                var seat_arr = self.getSeatsArray(pickup_id,return_id);
                                                if(seat_arr.length < number_of_tickets)
                                                {
                                                    var error_msg = self.opts.validation.invalid_seat;
                                                    seatsErrMsgBlock.html(error_msg.replace(/\{seats\}/g, number_of_tickets));
                                                    seatsErrMsgBlock.css('display', 'block');
                                                    valid = false;
                                                }
                                            }
				...
				var date = pjQ.$(this).attr('data-date'),
					...
					is_transfer = pjQ.$(this).attr('data-is_transfer'),
					is_transfer_str = is_transfer;
				if (parseInt(is_transfer) == 1 && type == 'return') {
				...
				var class_6 = 'col-lg-6 col-md-6 col-sm-6 col-xs-6';
				...
				hashBang("#!/" + hashString);
			}).on("click.bs", ".pjBrNavCurrency", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var $currency = pjQ.$(this).attr("data-currency"),
					$view = pjQ.$(this).attr("data-view");
				pjQ.$.get([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionCetCurrency"].join(""), {
					"locale": self.opts.locale,
					"hide": self.opts.hide,
					"index": self.opts.index,
					"currency": $currency
		...
		onReselect: function(pickup_id,return_id)
		{
                    var self = this;
                    var strIds = pickup_id.toString() + return_id.toString();
                    pjQ.$(".bsMapContainer_" + self.opts.index + strIds + " .bs-selected").each(function( index ) {
                        pjQ.$(this).removeClass('bs-selected');
                    });
                    pjQ.$('.bsSelectedSeatsLabel_' + self.opts.index + strIds).html('');
                    pjQ.$('.bs_selected_seats_' + self.opts.index + strIds).val('');
                    pjQ.$('#bsReSelect'+strIds).css('display','none');
		},
		loadSearch: function () {
			var self = this,
		...
		loadSeats: function () {
                    ...
                    pjQ.$.get([this.opts.folder, "index.php?controller=pjFrontPublic&action=pjActionSeats"].join(""), params).done(function (data) {
                        ...
                        var $form = pjQ.$('#bsSelectSeatsForm_' + self.opts.index);
                        pjQ.$('.bs_selected_bus').each(function(){
                            var bus_id = pjQ.$(this).val(),
                            mapBlock = pjQ.$(this).closest('.bsBusContainer').find('.bsMapContainer'),
                            pickup_id = mapBlock.data('pickup_id'),
                            return_id = mapBlock.data('return_id'),
                            return_bus_id = pjQ.$('#bs_return_selected_bus_' + self.opts.index).val(),
                            number_of_tickets = pjQ.$(this).closest('.bsBusContainer').find('.bs_selected_tickets').val();
                            if(('' != number_of_tickets) && ('0' != number_of_tickets)){
                                self.loadMap(bus_id,pickup_id,return_id);
                            }
                            pjQ.$.post([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionGetRoundtripPrice&bus_id=", bus_id, "&return_bus_id=", return_bus_id, "&is_return=","F","&pickup_id=", pickup_id, "&return_id=", return_id].join(""), $form.serialize()).done(function (data) {
                                pjQ.$('#bsRoundtripPrice_' + self.opts.index).html(data);
                            });
                        });

                        
                        pjQ.$('.bs_return_selected_bus').each(function(){
                            var bus_id = pjQ.$(this).val(),
                            mapBlock = pjQ.$(this).closest('.bsBusContainer').find('.bsMapContainer'),
                            pickup_id = mapBlock.data('pickup_id'),
                            return_id = mapBlock.data('return_id'),
                            return_bus_id = pjQ.$('#bs_return_selected_bus_' + self.opts.index).val(),
                            number_of_tickets = pjQ.$(this).closest('.bsBusContainer').find('.bs_return_selected_tickets').val();
		...
		loadMap: function(bus_id, pickup_id, return_id){
                    var self = this,
                        uniqueStr = self.opts.index.toString()+pickup_id.toString()+return_id.toString(),
                        params = {
                            "locale": this.opts.locale,
                            "hide": this.opts.hide,
                            "index": this.opts.index,
                    ...
                    $mapContaner = pjQ.$('.bsMapContainer_' + uniqueStr),
                    $selected_seats = pjQ.$('.bsSelectedSeatsLabel_' + uniqueStr);
                    $selected_seats.html('');
                    $selected_seats.parent().css('display', 'inline-block');

                    pjQ.$('.bs_selected_seats_' + uniqueStr).val('');
                    self.disableButtons.call(self);
                    pjQ.$.get([this.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionGetSeats"].join(""), params).done(function (data) {
                        ...
                        pjQ.$('.pjBsReturnSeatsBody').show();
                        pjQ.$('.pjBsReturnSeatsFoot').show();
                        pjQ.$('#bs_selected_tickets_' + uniqueStr).attr('data-map', 'T');
                ...
                loadReturnMap: function(bus_id, pickup_id, return_id){
                    var self = this,
                        strIds = pickup_id+''+return_id,
                        params = {
                            "locale": this.opts.locale,
                    ...
                    var $mapContaner = pjQ.$('#bsReturnMapContainer_' + self.opts.index + strIds),
                    $selected_seats = pjQ.$('#bsReturnSelectedSeatsLabel_' + self.opts.index + strIds);
                    $selected_seats.html('');
                    $selected_seats.parent().css('display', 'inline-block');
                    pjQ.$('#bs_return_selected_seats_' + self.opts.index + strIds).val('');
		...
		hideMap: function(pickup_id,return_id){
                    var self = this,
                        strIds = pickup_id + '' + return_id,
                        $mapContaner = pjQ.$('.bsMapContainer_' + self.opts.index + strIds),
                        $selected_seats = pjQ.$('.bsSelectedSeatsLabel_' + self.opts.index + strIds);
                    $mapContaner.html('');
		...
		getSeatsArray: function(pickup_id,return_id)
		{
                    var self = this,
                        selected_seats = pjQ.$('.bs_selected_seats_' + self.opts.index + pickup_id + return_id).val(),
                        seat_arr = Array();
                
                    if((undefined !== selected_seats) && selected_seats !== '')
                    {
		...
		getSeatsNameArray: function(pickup_id,return_id)
		{
                    var self = this,
                        selected_seats = pjQ.$('.bsSelectedSeatsLabel_' + self.opts.index + pickup_id + return_id).html(),
                        seat_arr = Array();
		...
		loadCheckout: function () {
                    ...
                    pjQ.$.get([this.opts.folder, "index.php?controller=pjFrontPublic&action=pjActionCheckout"].join(""), qs).done(function (data) {
                        ...
                        if (pjQ.$(".pjBsInputPhone").length > 0) {
                            pjQ.$(".pjBsInputPhone").intlTelInput({
                                autoHideDialCode: false,
                                nationalMode: false,
                                //separateDialCode: true,
                                onlyCountries: ['by', 'ru', 'ua'],
                                utilsScript: self.opts.folder + "app/web/js/utils.js"
                            });
                        }
                    });
		},
