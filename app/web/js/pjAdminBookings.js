...
(function ($, undefined) {
	$(function () {
		...
		if ($frmCreateBooking.length > 0 || $frmUpdateBooking.length > 0) {
			...
			$("input[name='multiselect_assigned_seats'], input[name='multiselect_assigned_return_seats']").live("click", function() {
				var $frm = $frmCreateBooking;
				if ($frmUpdateBooking.length > 0) {
					$frm = $frmUpdateBooking;
				}
				$.post("index.php?controller=pjAdminBookings&action=loadPassengers", $frm.serialize()).done(function (data) {
					$('.pjPassengerListsContainer').html(data);
				});
			})
		...
		$(document).on("focusin", ".datepick", function (e) {
			...
			$("#sub_total").closest(".pj-form-field-custom").find(".pj-form-field-icon-text-small").html($(this).val());
			$("#tax").closest(".pj-form-field-custom").find(".pj-form-field-icon-text-small").html($(this).val());
			$("#total").closest(".pj-form-field-custom").find(".pj-form-field-icon-text-small").html($(this).val());
			$("#deposit").closest(".pj-form-field-custom").find(".pj-form-field-icon-text-small").html($(this).val());
			$("#return_bus_id").trigger("change");
		}).on("click", "#is_transfer", function (e) {
			if (this.checked) {
				$("#transfer_booking_id").addClass("required");
				$("#transfer_booking_id").prop("disabled", false);
			} else {
				$("#transfer_booking_id").removeClass("required");
				$("#transfer_booking_id").prop("disabled", true);
			}
		});
