...
(function (window, undefined){
	...
	function calculateResultSumm(){
            var summ=0,
                price=0,
                qnt=0;
        
            pjQ.$('#commonTicketPanel .common-price').each(function(){
                price = parseFloat(pjQ.$(this).find('.tot-summ').text());
                qnt = parseInt(pjQ.$(this).find('.bsTicketExchangeSelect').val());
                summ += price*qnt;
            });
			...
			this.$container.on("click.bs", ".bsSelectorLocale", function (e) {
				...
				return false;
                            ...
                            var chosenQnt = pjQ.$(this).val(),
                                ticketType = pjQ.$(this).data('ticket-type');
                            pjQ.$('[data-ticket-type="'+ticketType+'"]').each(function(){
                                pjQ.$(this).val(chosenQnt);
                                if(pjQ.$(this).hasClass('bsDirectTicketSelect')){
                            ...
                            agregateSumm = false;
                            summ = calculateResultSumm();
                            pjQ.$('.pjBsTotalPrice .tot-summ').text(summ);
                        }).on("change.bs2", ".bsDirectTicketSelect", function (e) {
                            var $this = pjQ.$(this),
                                ...
                                total_tickets = 0,
                                seats = '',
                                $seat_label = pjQ.$("#bsSeats_" + uniqueStr),
                                $selected_bus = pjQ.$("#bs_selected_bus_" + uniqueStr),
                                selected_bus_id = $selected_bus.val(),
                                $selected_seats_label = pjQ.$('.bsSelectedSeatsLabel_' + uniqueStr),
                            ...
                            pjQ.$('#bs_selected_tickets_' + uniqueStr).val(total_tickets);
                            pjQ.$('#bs_has_map_' + uniqueStr).val(seat_map);
                            ...
                            if(total_tickets == 0)
                            {
                                ...
                                if(seat_map == 'F')
                                {
                                    ...
                                    $selected_seats_label.parent().css('display', 'none');
                                    pjQ.$('#bs_selected_seats_' + uniqueStr).val(selected_id_arr.join("|"));
                                }
                            }
                            ...
                            var $this = pjQ.$(this),
                                ...
                                total_tickets = 0,
                                seats = '',
                                $seat_label = pjQ.$("#bsReturnSeats_" + uniqueStr),
                                $selected_bus = pjQ.$("#bs_return_selected_bus_" + uniqueStr),
                                selected_bus_id = $selected_bus.val(),
                            ...
                            if(true !== agregateSumm){
                                pjQ.$.post([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionGetRoundtripPrice&bus_id=", bus_id, "&return_bus_id=", return_bus_id, "&is_return=","T","&pickup_id=", pickup_id, "&return_id=", return_id].join(""), $form.serialize()).done(function (data) {
                                    pjQ.$('#bsRoundtripPrice_' + self.opts.index).html(data);
                                });
                            }				
                            if(selected_bus_id != '')
                            ...
                            pjQ.$('#bs_return_selected_tickets_' + uniqueStr).val(total_tickets);
                            pjQ.$('#bs_return_has_map_' + uniqueStr).val(seat_map);
                            self.onReselect(pickup_id,return_id);
                            if(total_tickets != 1)
                            {
                                seats = total_tickets + ' ' + self.opts.labels.seats;
                            ...
                            if(total_tickets == 0)
                            {
                                ...
                                if(seat_map == 'F')
                                {
                                    ...
                                    $selected_seats_label.html(selected_name_arr.join(", "));
                                    $selected_seats_label.parent().css('display', 'none');
                                    pjQ.$('#bs_return_selected_seats_' + uniqueStr).val(selected_id_arr.join("|"));
                                }
                            }
                    ...
                    pjQ.$.get([this.opts.folder, "index.php?controller=pjFrontPublic&action=pjActionSeats"].join(""), params).done(function (data) {
                        ...
                        pjQ.$('.bs_selected_bus').each(function(){
                            ...
                            pjQ.$.post([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionGetRoundtripPrice&bus_id=", bus_id, "&return_bus_id=", return_bus_id, "&is_return=","F","&pickup_id=", pickup_id, "&return_id=", return_id].join(""), $form.serialize()).done(function (data) {
                                ...
                                if(0 !== pjQ.$('#commonTicketPanel').length){
                                    var summ = calculateResultSumm();
                                    pjQ.$('.pjBsTotalPrice .tot-summ').text(summ);
                                }
     ...
     console.log(data);                   
		...
		reCalculatingTickets: function($this, max_seats, bus_id)
		{
                    var current_value = parseInt($this.val(), 10),
                        number_of_seats = parseInt($this.closest('.bsBusContainer').find('.bs_number_of_seats').val(), 10);
