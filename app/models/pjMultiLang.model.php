...
class pjMultiLangModel extends pjAppModel
{
	...
	public function updateMultiLang($data, $foreign_id, $model, $source=NULL)
	{
        ...
        public function getMultilangArr($arrForeignId, $model){
            $arr = array();
            $_arr = $this
                ->reset()
                ->whereIn('foreign_id', $arrForeignId)
                ->where('model', $model)
                ->findAll()
                ->getData();
            ...
            foreach ($_arr as $_v)
            {
                $arr[$_v['foreign_id']][$_v['locale']][$_v['field']] = $_v['content'];
                $arr[$_v['foreign_id']][$_v['locale']]['id'] = $_v['foreign_id'];
            }
