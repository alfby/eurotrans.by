...
class pjPriceModel extends pjAppModel
{
	...
	public function getTicketPrice($bus_id, $pickup_id, $return_id, $booked_data, $option_arr, $locale_id, $is_return, $field_price='price', $is_transfer=0)
	{
            $sub_total = 0;
            $tax = 0;
            $total = 0;
            ...
            foreach($ticket_arr as $k => $v)
            {
                ...
                {
                    $discount = 0;
                    if(isset($v['discount']) && (float) $v['discount'] > 0 && $is_return == 'T' && $is_transfer == 0)
                    {
                        $discount = $v['discount'];
                    }
                    $price = $v[$field_price] - ($v[$field_price] * $discount / 100);
        ...
        public function getTicketArr($bus_id,$pickup_id,$return_id,$is_return){
            return $this
                ->reset()
                ->select('t1.*')
                ->where( 't1.bus_id',$bus_id)
