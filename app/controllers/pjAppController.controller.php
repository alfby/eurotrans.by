...
class pjAppController extends pjController
{
	...
	public $defaultTicketCurrencies = array(
		'BYN' => 'price',
		'RUB' => 'price_2'
	);
	...
	public function getData($option_arr, $booking_arr, $salt, $locale_id)
	{
		...
		$option_arr['o_currency'] = $booking_arr['currency'];
		$seats = '';
		...
		$booked_passenger_arr = pjBookingPassengerModel::factory()
			->select('t1.*, t2.content AS country_title')
			->join('pjMultiLang', "t2.model='pjCountry' AND t2.foreign_id=t1.country_id AND t2.field='name' AND t2.locale='".$locale_id."'", 'left outer')
			->where('booking_id', $booking_arr['id'])
			->findAll()
			->getDataPair('seat_id');
		if(!empty($booked_seat_id_arr))
		{
			...
			$seats_arr = array();
			$i = 0;
			foreach ($selected_seat_arr as $seat_id => $seat_name) {
				if ($i == 0) {
					$seats_arr[] = '#'.$seat_name.' - '.__('front_label_passengers_first_name', true).': '.@$booked_passenger_arr[$seat_id]['first_name'].'; '.__('front_label_passengers_last_name', true).': '.@$booked_passenger_arr[$seat_id]['last_name'].'; '.__('front_label_passengers_phone_1', true).': '.@$booked_passenger_arr[$seat_id]['phone_1'].'; '.__('front_label_passengers_phone_2', true).': '.@$booked_passenger_arr[$seat_id]['phone_2'].'; '.__('front_label_passengers_passpor_id', true).': '.@$booked_passenger_arr[$seat_id]['passpor_id'].'; '.__('front_label_passengers_country', true).': '.@$booked_passenger_arr[$seat_id]['country_title'].'; '.__('front_label_passengers_email', true).': '.@$booked_passenger_arr[$seat_id]['email'];
				} else {
					$seats_arr[] = '#'.$seat_name.' - '.__('front_label_passengers_first_name', true).': '.@$booked_passenger_arr[$seat_id]['first_name'].'; '.__('front_label_passengers_last_name', true).': '.@$booked_passenger_arr[$seat_id]['last_name'].'; '.__('front_label_passengers_passpor_id', true).': '.@$booked_passenger_arr[$seat_id]['passpor_id'].'; '.__('front_label_passengers_country', true).': '.@$booked_passenger_arr[$seat_id]['country_title'];
				}
				$i++;
			}
			$seats = join("<br/>", $seats_arr);
		}
	...
	public function getTemplate($option_arr, $booking_arr, $salt, $locale_id)
	{
		...
		if(!empty($booked_seat_id_arr))
		{
			...
			$seats_arr = array();
			foreach ($selected_seat_arr as $seat_id => $seat_name) {
				$seats_arr[] = '#'.$seat_name.': '.@$booked_passenger_arr[$seat_id]['first_name']." ".@$booked_passenger_arr[$seat_id]['last_name'];
			}
			$seats = join("; ", $seats_arr);
		}
		$row = array();
		if (isset($booking_arr['tickets']))
		{
	...
	public function getBusList($pickup_id, $return_id, $bus_id_arr, $booking_period, $booked_data, $date, $is_return) {
		...
		$pickup_location = $pjCityModel->reset ()->select ( 't1.*, t2.content as name' )->join ( 'pjMultiLang', "t2.model='pjCity' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='" . $localeId . "'", 'left outer' )->find ( $pickup_id )->getData ();
		$return_location = $pjCityModel->reset ()->select ( 't1.*, t2.content as name' )->join ( 'pjMultiLang', "t2.model='pjCity' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='" . $localeId . "'", 'left outer' )->find ( $return_id )->getData ();
		
		$ticket_columns = 0;
		$booking_date = pjUtil::formatDate ( $date, $this->option_arr ['o_date_format'] );
		
		$bus_arr = pjBusModel::factory ()->join ( 'pjMultiLang', "t2.model='pjRoute' AND t2.foreign_id=t1.route_id AND t2.field='title' AND t2.locale='" . $localeId . "'", 'left outer' )->join ( 'pjBusType', "t3.id=t1.bus_type_id", 'left outer' )->select ( " t1.*, t2.content AS route, t3.seats_map" )->where ( "(t1.id IN(" . join ( ',', $bus_id_arr ) . "))" )->index ( "FORCE KEY (`bus_type_id`)" )->orderBy ( "route asc" )->findAll ()->getData ();
		$location_id_arr = array ();
                ...
                foreach ( $bus_arr as $k => $bus ) {
                    ...
                    if(!empty($temp_location_id_arr)) {
                        $ticket_price_arr = $pjPriceModel->getTicketPrice ( $bus_id, $pickup_id, $return_id, $booked_data, $this->option_arr, $localeId, $is_return );
