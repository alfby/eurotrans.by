					...
					if(isset($_POST['index_arr']) && $_POST['index_arr'] != '')
					{
						...
						foreach($index_arr as $k => $v)
						{
							if(strpos($v, 'bs') !== false)
							{
								...
								if (isset($_POST['discount'][$v])) {
									$t_data['discount'] = $_POST['discount'][$v];
								}
						...
						foreach($location_arr as $col)
						{
							if($j > 1)
							{
								...
								if($cnt == 0)
								{
									...
									foreach ($this->defaultTicketCurrencies as $key => $val) {
   										$price = isset($_POST[$val.'_' . $row['city_id'] . '_' . $col['city_id']]) ? $_POST[$val.'_' . $row['city_id'] . '_' . $col['city_id']] : '';
										//$price = $_POST[$val.'_' . $row['city_id'] . '_' . $col['city_id']];
										if($price != '')
										{
											if (!is_numeric($price)) 
											{
				...
				if(count($ticket_arr) > 0)
				{
					...
					if(!empty($location_id_arr))
					{
						...
						if(!empty($_price_arr))
						{
							...
							{
								$price_arr[$v['from_location_id'] . '_' . $v['to_location_id']]['price'] = $v['price'];
								$price_arr[$v['from_location_id'] . '_' . $v['to_location_id']]['price_2'] = $v['price_2'];
							}
						}
					}
					...
					if(!empty($location_id_arr))
					{
						...
						if(!empty($_price_arr))
						{
							foreach($_price_arr as $v)
							{
								$return_price_arr[$v['from_location_id'] . '_' . $v['to_location_id']]['price'] = $v['price'];
								$return_price_arr[$v['from_location_id'] . '_' . $v['to_location_id']]['price_2'] = $v['price_2'];
							}
						}
					}
