...
class pjAdminCities extends pjAdmin
{
	...
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			{
                            if(isset($_POST['av_cities']['id']) && !empty($_POST['av_cities']['id'])){
                                $_POST['dest_cities_list_id'] = implode(self::DELIMETER,$_POST['av_cities']['id']);
                            }
                            ...
                            $pjCityModel = pjCityModel::factory();
                            $arCities = $pjCityModel
                                ->reset()
                                ->select('t1.*, t2.content as name,t2.locale')
                                ->join('pjMultiLang', "t2.model='pjCity' AND t2.foreign_id=t1.id AND t2.field='name'", 'left outer')
	...
	public function pjActionUpdate()
	{
            ...
            if ($this->isAdmin() || $this->isEditor())
            {
                if (isset($_POST['city_update']))
                {
                    ...
                    $arDestCitiesId = explode(self::DELIMETER,$arr['dest_cities_list_id']);
                    $arrDestCities = $pjMultiLangModel->getMultilangArr($arDestCitiesId, 'pjCity');
                    $this->set('dest_cities',$arrDestCities);
