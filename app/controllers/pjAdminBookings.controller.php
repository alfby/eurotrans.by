...
class pjAdminBookings extends pjAdmin
{
			...
			if (isset($_POST['booking_create']))
			{
				...
				if ($id !== false && (int) $id > 0)
				{
					...
					foreach($location_pair as $pair)
					{
						...
						foreach($ticket_arr as $v)
						{
							...
							for($i = 1; $i <= $qty; $i ++)
							{
								...
								$selected_ticket_arr[$seat_id_arr[$k]] = $v['ticket_id'];
								$k++;
							}
						}
					}
					...
					foreach ($seat_id_arr as $seat_id) {
						...
						$data ['ticket_id'] = $selected_ticket_arr[$seat_id];
						$data ['first_name'] = @$_POST['p_first_name'][$seat_id];	
						$data ['last_name'] = @$_POST['p_last_name'][$seat_id];	
						$data ['phone_1'] = @$_POST['p_phone_1'][$seat_id];	
						$data ['phone_2'] = @$_POST['p_phone_2'][$seat_id];	
						$data ['passpor_id'] = @$_POST['p_passpor_id'][$seat_id];
						$data ['country_id'] = @$_POST['p_country_id'][$seat_id];
						$data ['email'] = @$_POST['p_email'][$seat_id];					
					...
					if (isset($_POST['is_return']) && isset($_POST['return_bus_id'])) 
					{
						...
						{	
							...
							foreach ($seat_id_arr as $seat_id) {
								$data = array ();
								$data ['booking_id'] = $return_booking_id;
								$data ['seat_id'] = $seat_id;
								$data ['ticket_id'] = $selected_ticket_arr[$seat_id];
								$data ['first_name'] = @$_POST['r_first_name'][$seat_id];	
								$data ['last_name'] = @$_POST['r_last_name'][$seat_id];	
								$data ['phone_1'] = @$_POST['r_phone_1'][$seat_id];	
								$data ['phone_2'] = @$_POST['r_phone_2'][$seat_id];	
								$data ['passpor_id'] = @$_POST['r_passpor_id'][$seat_id];
								$data ['country_id'] = @$_POST['r_country_id'][$seat_id];
								$data ['email'] = @$_POST['r_email'][$seat_id];						
								$pjBookingPassengerModel->reset()->setAttributes($data)->insert();
							}
						}
					}
				...
				if(isset($_GET['bus_id']) && (int) $_GET['bus_id'] > 0)
				{
					...
					$ticket_arr = pjPriceModel::factory()
						...
						->select("t1.*, t2.seats_count, t3.content as ticket, t2.discount")
						->where('t1.bus_id', $bus_id)
						->where('t1.from_location_id', $pickup_id)
						->where('t1.to_location_id', $return_id)
						->where('is_return = "F"')
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['booking_update']))
			{
				...
				$data['booking_time'] = join(" - ", $bt_arr);
				if (isset($_POST['is_transfer'])) {
					$data['transfer_booking_id'] = $_POST['transfer_booking_id'];
					$data['is_transfer'] = 1;
				} else {
					$data['transfer_booking_id'] = ':NULL';
					$data['is_transfer'] = 0;
				}
				$pjBookingModel->reset()->where('id', $_POST['id'])->limit(1)->modifyAll(array_merge($_POST, $data));
				
				if (isset($_POST['is_transfer']) && (int)$_POST['transfer_booking_id'] > 0) {
					$pjBookingModel->reset()->where('id', $_POST['transfer_booking_id'])->limit(1)->modifyAll(array('is_transfer' => 1 ,'transfer_booking_id' => $_POST['id']));
				} else {
					$pjBookingModel->reset()->where('transfer_booking_id', $_POST['id'])->limit(1)->modifyAll(array('is_transfer' => 0 ,'transfer_booking_id' => 0));
				}
				...
				$pjBookingPassengerModel = pjBookingPassengerModel::factory();
				$pjBookingPassengerModel->where('booking_id', $_POST['id'])->eraseAll();
				foreach ($seat_id_arr as $seat_id) {
					$data = array ();
					$data ['booking_id'] = $_POST['id'];
					$data ['seat_id'] = $seat_id;
				...
				/*$ticket_pair_arr = $pjBookingSeatModel->reset()->select("t1.ticket_id, COUNT(DISTINCT seat_id) as qty")->where('booking_id', $_GET['id'])->where('is_return = "F"')->groupBy("t1.ticket_id")->findAll()->getDataPair("ticket_id", 'qty');*/
				$ticket_pair_arr = pjBookingTicketModel::factory()->where('booking_id', $_GET['id'])->groupBy("t1.ticket_id")->findAll()->getDataPair("ticket_id", 'qty');
				...
				$seat_passenger_arr = pjBookingPassengerModel::factory()->where('booking_id', $_GET['id'])->findAll()->getDataPair("seat_id");
				$this->set('seat_passenger_arr', $seat_passenger_arr);
				
				$this->set('transfer_booking_arr', pjBookingModel::factory()->select('t1.id, t1.uuid')->reset()->where('t1.id!='.(int)$_GET['id'])->findAll()->getData());
	...
	public function loadPassengers() {
		$this->setAjax(true);
		if ($this->isXHR()) {
			$seat_pair_arr = $return_seat_pair_arr = $selected_seats = $seat_passenger_arr = array();
			$booking_arr = array();
			if (isset($_POST['id']) && (int)$_POST['id'] > 0) {
				$booking_arr = pjBookingModel::factory()->find($_POST['id'])->getData();
			}
			if ($booking_arr) {
				if ($booking_arr['is_return'] == 'F') {
					$seat_pair_arr = isset($_POST['assigned_seats']) ? $_POST['assigned_seats'] : array();
				} else {
					$seat_pair_arr = isset($_POST['selected_seats']) ? explode("|", $_POST['selected_seats']) : array();
				}
			} else {
				if (isset($_POST['selected_seats']) && $_POST['selected_seats']) {
					$seat_pair_arr = explode("|", $_POST['selected_seats']);
				} elseif (isset($_POST['assigned_seats']) && $_POST['assigned_seats']) {
					$seat_pair_arr = $_POST['assigned_seats'];
				}
			}
			if(!empty($seat_pair_arr))
			{
			...
			if (isset($_POST['return_selected_seats']) && $_POST['return_selected_seats']) {
				$return_seat_pair_arr = explode("|", $_POST['return_selected_seats']);
			} elseif (isset($_POST['assigned_return_seats']) && $_POST['assigned_return_seats']) {
				$return_seat_pair_arr = $_POST['assigned_return_seats'];
			}
			
			if(!empty($return_seat_pair_arr))
			{
				$return_selected_seats = pjSeatModel::factory()->reset()->whereIn('id', $return_seat_pair_arr)->findAll()->getDataPair("id", 'name');
			}
			
			if (isset($_POST['id']) && (int)$_POST['id'] > 0) {
				$seat_passenger_arr = pjBookingPassengerModel::factory()->where('booking_id', $_POST['id'])->findAll()->getDataPair("seat_id");
			}
			$this->set('selected_seats', $selected_seats);
			$this->set('return_selected_seats', $return_selected_seats);
			$this->set('seat_passenger_arr', $seat_passenger_arr);
