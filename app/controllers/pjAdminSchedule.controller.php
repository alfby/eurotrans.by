		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if(isset($_GET['bus_id']) && isset($_GET['date']))
			{
				...
				{					
					...
					$booking_arr[$k]['passenger_name'] = pjBookingPassengerModel::factory()->reset()
						->reset()
						->where("t1.booking_id", $v['id'])
						->findAll()
						->getDataPair("seat_id");
	...
	public function pjActionSeats()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			foreach($booking_arr as $v)
			{
				...
				$passenger_name = pjBookingPassengerModel::factory()->reset()
						->reset()
						->select('t1.first_name, t1.last_name')
						->where("t1.booking_id", $v['id'])
						->where('t1.seat_id', $v['seat_id'])
						->limit(1)
						->findAll()
						->getData();
				$v['passenger_name'] = $passenger_name ? ($passenger_name[0]['first_name']." ".$passenger_name[0]['last_name']) : '';
