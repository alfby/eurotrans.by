					...
					if(isset($_POST['index_arr']) && $_POST['index_arr'] != '')
					{
						...
						foreach($index_arr as $k => $index)
						{
							if(isset($_POST['city_id_' . $index]) && (int) $_POST['city_id_' . $index] > 0)
							{
								...
								$data['is_departure'] = isset($_POST['is_departure_' . $index]) ? 1 : 0;
								$data['is_arrival'] = isset($_POST['is_arrival_' . $index]) ? 1 : 0;
								//$data['exclude_from_search'] = isset($_POST['exclude_from_search_' . $index]) ? 1 : 0;
								$data['order'] = $k + 1;
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			if (isset($_POST['route_update']))
			{
				...
				$arr['route_stop'] = pjRouteCityModel::factory()->reset()->where('t1.route_id', $arr['id'])->findAll()->getDataPair('city_id');
