...
class pjFrontEnd extends pjFront
{
	...
	public function pjActionCheck()
	{
            if ($this->isXHR() || isset($_GET['_escaped_fragment_']))
            {
                ...
                if($_GET['pickup_id'] !== $_GET['return_id'])
                {
                    ...
                    $booked_data = array();
                    $date = isset($_GET['return_date']) ? pjUtil::formatDate($_GET['date'], $this->option_arr['o_date_format']) : false;
                    $return_date = isset($_GET['return_date']) ? pjUtil::formatDate($_GET['date'], $this->option_arr['o_date_format']) : false;
                    ...
                    $isDirectBusAvailable = $this->checkIsBusAvailable($pickup_id, $return_id, $bus_id_arr, $booking_period, $_GET['date'], 'F');
                    if(false === $isDirectBusAvailable)//has no direct bus
                    {
                        $location_arr = $pjCityModel
                            ->select('t1.*, t2.content as name')
                            ->join('pjMultiLang', "t2.model='pjCity' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='".$localeId."'", 'left outer')
                            ->where("t1.id IN(
                        ...
                        if ($location_arr) {//if has transfer
                            ...
                            foreach ($location_arr as $location) {
                                ...
                                if ($bus_id_before_transfer_arr) {
                                    ...
                                    $isAvailable = $this->checkIsBusAvailable($after_transfer_pickup_id, $after_transfer_return_id, $bus_id_after_transfer_arr, $booking_period, $_GET['date'], 'F');
                                    if(false === $isAvailable){//there is no available after transfer bus
                                        $resp['code'] = 100;
                                        pjAppController::jsonResponse($resp);
                                    }
                                    if ($bus_id_after_transfer_arr) {
                                        //break;
                                    }
                                }
                            }
                    ...
                    if (('T' == $is_return) && (0 == $is_transfer))
                    {
                        ...
                        if(('T' == $is_return) && (1 == $is_transfer)){
                            ...
                            if($bus_id_return_before_transfer_arr){
                                $isAvailable = $this->checkIsBusAvailable($return_before_transfer_pickup_id, $return_before_transfer_return_id, $bus_id_return_before_transfer_arr, $booking_period, $_GET['return_date'], 'F');
                                if($isAvailable){
                            ...
                            if($bus_id_return_after_transfer_arr){
                                $isAvailable = $this->checkIsBusAvailable($return_after_transfer_pickup_id, $return_after_transfer_return_id, $bus_id_return_after_transfer_arr, $booking_period, $_GET['return_date'], 'F');
                                if($isAvailable){
                    ...
                    if(!isset($_GET['final_check']))
                    {
                        ...
                        if (1 == $is_transfer) {
                            ...
                            $this->_set('return_date', $_GET['date']);
                        } else {
                            if ('T' == $is_return)
                            {
                                $this->_set('return_bus_id_arr', $return_bus_id_arr);
                        ...
                        if ($STORE['is_transfer'] == 1) {
                            ...
                            if ($is_valid) {
                                ...
                                $avail_arr = $this->getBusAvailability($STORE['booked_data']['return_bus_id'], $STORE, $this->option_arr);
                                $booked_seat_arr = $avail_arr['booked_seat_arr'];
                                $seat_id_arr = explode("|", $STORE['booked_data']['selected_seats']);
                                $intersect = array_intersect($booked_seat_arr, $seat_id_arr);
                                if(!empty($intersect))
	...
	public function pjActionSaveBooking() {
		...
		if ($this->isXHR ()) {
			...
			if ($is_transfer == 1) {
				$return_location = $pjCityModel->reset ()->select ( 't1.*, t2.content as name' )->join ( 'pjMultiLang', "t2.model='pjCity' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='" . $this->getLocaleId () . "'", 'left outer' )->find ( $before_transfer_return_id )->getData ();
			} else {
			...
			if ($is_transfer == 1) {
				$_arr = $pjBusLocationModel->reset ()->where ( 'bus_id', $bus_id )->where ( "location_id", $before_transfer_return_id )->limit ( 1 )->findAll ()->getData ();	
			} else {
			...
			if ($is_transfer == 1) {
				$data ['pickup_id'] = $before_transfer_pickup_id;
				$data ['return_id'] = $before_transfer_return_id;
			} else {
				$data ['pickup_id'] = $pickup_id;
				$data ['return_id'] = $return_id;
			}
			$data ['is_return'] = $is_transfer == 0 ? $is_return : 'F';
			...
			if ($id !== false && ( int ) $id > 0) {
				...
				if ($is_return == 'T') {
					...
					$pjBusLocationModel = pjBusLocationModel::factory ();
					if ($is_transfer == 1) {
						$_arr = $pjBusLocationModel->where ( 'bus_id', $child_bus_arr ['id'] )->where ( "location_id", $after_transfer_pickup_id )->limit ( 1 )->findAll ()->getData ();
					} else {
					...
					if ($is_transfer == 1) {
						$_arr = $pjBusLocationModel->reset ()->where ( 'bus_id', $child_bus_arr ['id'] )->where ( "location_id", $after_transfer_return_id )->limit ( 1 )->findAll ()->getData ();
					} else {
					...
					$data ['uuid'] = time () + 1;
					if ($is_transfer == 1){
						$data ['pickup_id'] = $after_transfer_pickup_id;
						$data ['return_id'] = $after_transfer_return_id;
					} else {
					...
					if ($back_insert_id !== false && ( int ) $back_insert_id > 0) {
						if ($is_transfer == 1) {
                                                    $pjBookingModel
                                                        ...
                                                        ->modify(array(
                                                            'is_transfer' => 1 ,
                                                            'transfer_booking_id' => $back_insert_id
                                                        ));

                                                    $pjBookingModel
                                                        ->reset()
                                                        ->set('id', $back_insert_id)
                                                        ->modify(array(
                                                            'is_transfer' => 1 ,
                                                            'transfer_booking_id' => $id 
                                                        ));
						} else {
				...
				if ($is_transfer == 1) {
					$ticket_arr = pjPriceModel::factory ()->select ( "t1.*" )->where ( 't1.bus_id', $bus_id )->where ( 't1.from_location_id', $before_transfer_pickup_id )->where ( 't1.to_location_id', $before_transfer_return_id )->where ( 'is_return = "F"' )->findAll ()->getData ();
					$location_arr = pjRouteCityModel::factory ()->getLocations ( $bus_arr ['route_id'], $before_transfer_pickup_id, $before_transfer_return_id );
				} else {
				...
				foreach ($seat_id_arr as $seat_id) {
					$data = array ();
					$data ['booking_id'] = $id;
					$data ['seat_id'] = $seat_id;
					$data ['ticket_id'] = $pickup_ticket_arr[$seat_id];
					$data ['first_name'] = @$FORM['p_first_name'][$seat_id];	
					$data ['last_name'] = @$FORM['p_last_name'][$seat_id];	
					$data ['phone_1'] = @$FORM['p_phone_1'][$seat_id];	
					$data ['phone_2'] = @$FORM['p_phone_2'][$seat_id];	
					$data ['passpor_id'] = @$FORM['p_passpor_id'][$seat_id];
					$data ['country_id'] = @$FORM['p_country_id'][$seat_id];
					$data ['email'] = @$FORM['p_email'][$seat_id];					
					$pjBookingPassengerModel->reset()->setAttributes($data)->insert();
				}
				
				if ($is_return == 'T') {
					if ($is_transfer == 1) {
						$ticket_arr = pjPriceModel::factory ()->select ( "t1.*, t3.discount" )
							->join ( 'pjBus', 't1.bus_id = t2.id', 'left' )
							->join('pjTicket', 't1.ticket_id = t3.id', 'left')
							->where ( 't1.bus_id', $return_bus_id )
							->where ( 't1.from_location_id', $after_transfer_pickup_id )
							->where ( 't1.to_location_id', $after_transfer_return_id )
							->where ( 'is_return = "F"' )
							->findAll ()
							->getData ();
						$location_arr = pjRouteCityModel::factory ()->getLocations ( $child_bus_arr ['route_id'], $after_transfer_pickup_id, $after_transfer_return_id );
					} else {
						$ticket_arr = pjPriceModel::factory ()->select ( "t1.*, t3.discount" )
							->join ( 'pjBus', 't1.bus_id = t2.id', 'left' )
							->join('pjTicket', 't1.ticket_id = t3.id', 'left')
							...
							->findAll ()
							->getData ();
						$location_arr = pjRouteCityModel::factory ()->getLocations ( $child_bus_arr ['route_id'], $return_id, $pickup_id );
					}
					$location_pair = array ();
					for($i = 0; $i < count ( $location_arr ); $i ++) {
						$j = $i + 1;
					...
					foreach ( $ticket_arr as $k => $v ) {
						if (isset ( $booked_data ['return_ticket_cnt_' . $v ['ticket_id']] ) && $booked_data ['return_ticket_cnt_' . $v ['ticket_id']] > 0) {
							...
							$data ['is_return'] = $is_transfer == 1 ? 'F' : 'T';
							$pjBookingTicketModel->reset ()->setAttributes ( $data )->insert ();
						}
					}
					$return_ticket_arr = array();
							...
							if (isset ( $booked_data ['return_ticket_cnt_' . $v ['ticket_id']] ) && $booked_data ['return_ticket_cnt_' . $v ['ticket_id']] > 0) {
								...
								if ($qty > 0) {
									for($i = 1; $i <= $qty; $i ++) {
										...
										$pjBookingSeatModel->reset ()->setAttributes ( $data )->insert ();
										$return_ticket_arr[$seat_id_arr[$kk]] = $v['ticket_id'];
										$kk ++;
									}
								}
							}
					...
					foreach ($seat_id_arr as $k => $seat_id) {
						if ($is_transfer == 0) {
							$first_name = @$FORM['r_first_name'][$seat_id];
							$last_name = @$FORM['r_last_name'][$seat_id];
							$phone_1 = @$FORM['r_phone_1'][$seat_id];
							$phone_2 = @$FORM['r_phone_2'][$seat_id];
							$passpor_id = @$FORM['r_passpor_id'][$seat_id];
							$country_id = @$FORM['r_country_id'][$seat_id];
							$email = @$FORM['r_email'][$seat_id];
						} else {
							$idx = 0;
							foreach ($FORM['p_first_name'] as $sid => $item) {
								if ($idx == $k) {
									$first_name = @$FORM['p_first_name'][$sid];
									$last_name = @$FORM['p_last_name'][$sid];
									$phone_1 = @$FORM['p_phone_1'][$sid];
									$phone_2 = @$FORM['p_phone_2'][$sid];
									$passpor_id = @$FORM['p_passpor_id'][$sid];
									$country_id = @$FORM['p_country_id'][$sid];
									$email = @$FORM['p_email'][$sid];
									break;
								}
								$idx++;
							}
						...
						$data ['booking_id'] = $back_insert_id;
						$data ['seat_id'] = $seat_id;
						$data ['ticket_id'] = $return_ticket_arr[$seat_id];
						$data ['first_name'] = $first_name;	
						$data ['last_name'] = $last_name;	
						$data ['phone_1'] = $phone_1;	
						$data ['phone_2'] = $phone_2;	
						$data ['passpor_id'] = $passpor_id;
				...
				if ($is_return == 'T') {
					$return_arr = $pjBookingModel->reset ()->select ( 't1.*, t2.departure_time, t2.arrival_time, t3.content as route_title, t4.content as from_location, t5.content as to_location' )->join ( 'pjBus', "t2.id=t1.bus_id", 'left outer' )->join ( 'pjMultiLang', "t3.model='pjRoute' AND t3.foreign_id=t2.route_id AND t3.field='title' AND t3.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjMultiLang', "t4.model='pjCity' AND t4.foreign_id=t1.pickup_id AND t4.field='name' AND t4.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjMultiLang', "t5.model='pjCity' AND t5.foreign_id=t1.return_id AND t5.field='name' AND t5.locale='" . $this->getLocaleId () . "'", 'left outer' )->find ( $back_insert_id )->getData ();
					
					$return_tickets = pjBookingTicketModel::factory ()->join ( 'pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjTicket', "t3.id=t1.ticket_id", 'left' )->select ( 't1.*, t2.content as title' )->where ( 'booking_id', $back_insert_id )->findAll ()->getData ();
	...
	public function pjActionGetLocations()
	{
            ...
            //$pjRouteDetailModel = pjRouteDetailModel::factory();
            $pjRouteCityModel = pjRouteCityModel::factory();
            $locId = $this->getLocaleId();

            if(isset($_GET['pickup_id']))
            {
                $location_arr = $pjCityModel->getDestCities($_GET['pickup_id'],$locId,',');
            }
                ...
                if (isset($_SESSION[$this->defaultStore]) && count($_SESSION[$this->defaultStore]) > 0 && $this->isBusReady() == true)
                {
                    ...
                    if($bus_id > 0)
                    {
                        if (1 === (int)$is_transfer) {
                            ...
                            $direct_bus_list = $this->getBusList($pickup_id, $return_id, $bus_id_arr, $booking_period, $booked_data, $date, 'F');
                            if(isset($direct_bus_list['bus_arr'][0]['seats_available'])){// seats before transfer are available
                                $minSeats = getMinQuant($direct_bus_list['bus_arr'],'seats_available');
                                if(0 === $minSeats){//seats brefore transfer are not available
                                    $pickup_id = $this->_get('after_transfer_pickup_id');
                                    $return_id = $this->_get('after_transfer_return_id');
                                }
                            }
                        }
                        
                        $ticket_price_arr = $pjPriceModel->getTicketPrice($bus_id, $pickup_id, $return_id, $_POST, $this->option_arr, $this->getLocaleId(), 'F', $field_price, $is_transfer);
                        $this->set('price_arr', $ticket_price_arr);
                    }
                    if($return_bus_id > 0 && ($is_return == "T" || $is_transfer == 1))
                    {
                        if ((int)$is_transfer == 1) {
                            $pickup_id = $this->_get('after_transfer_pickup_id');
                            $return_id = $this->_get('after_transfer_return_id');
                        }
                        if('T' === $is_return){
                            if(1 === $is_transfer){//direct trip with transfer
                                $return_ticket_price_arr = $pjPriceModel->getTicketPrice($return_bus_id, $pickup_id, $return_id, $_POST, $this->option_arr, $this->getLocaleId(), 'T', $field_price, $is_transfer);
                            }elseif(0 === $is_transfer){
                                $return_ticket_price_arr = $pjPriceModel->getTicketPrice($return_bus_id, $return_id, $pickup_id, $_POST, $this->option_arr, $this->getLocaleId(), 'T', $field_price, $is_transfer);
                            }
                        }else{
                            $return_ticket_price_arr = $pjPriceModel->getTicketPrice($return_bus_id, $pickup_id, $return_id, $_POST, $this->option_arr, $this->getLocaleId(), 'T', $field_price, $is_transfer);
                        }
	...
	public function pjActionConfirmSend($option_arr, $booking_arr, $salt, $opt)
	{
            ...
            $tokens = pjAppController::getData($option_arr, $booking_arr, PJ_SALT, $locId);
            $pjMultiLangModel = pjMultiLangModel::factory();
            $pjBookingPassengerModel = pjBookingPassengerModel::factory();
    ...
    public function checkIsBusAvailable($pickup_id,$return_id,array $bus_id_arr,$booking_period,$date,$is_return){
        if(!empty($bus_id_arr)){
            $bus_list = $this->getBusList($pickup_id, $return_id, $bus_id_arr, $booking_period, array(), $date, $is_return);
