...
class pjUtil extends pjToolkit
{
	...
	public static function calDuration($start_time, $end_time)
	{
        ...
        public static function getDuration($departure_time,$arrival_time)
        {
            $duration_arr = self::calDuration($departure_time, $arrival_time);
        ...
        public static function formBusArray(array $bus_arr, array $pickup_arr, array $return_arr, $timeFormat, $optionTimeFormat){
            if(!empty($pickup_arr)){
                $bus_arr['departure_time'] = self::formatTime($pickup_arr[0]['departure_time'], $timeFormat, $optionTimeFormat);
            }
            if(!empty($return_arr)){
                $bus_arr['arrival_time'] = self::formatTime($return_arr[0]['arrival_time'], $timeFormat, $optionTimeFormat);
            }
            if(!empty($pickup_arr) && !empty($return_arr))
            {
                $bus_arr['duration'] = self::getDuration($pickup_arr[0]['departure_time'], $return_arr[0]['arrival_time']);
            }
	...
	public static function toMomemtJS($format)
	{
        ...
        public static function formPriceArray(array $ticket_price_arr){
            $ticketsTotal = array(
                'sub_total' => 0,
                'tax' => 0,
                'total' => 0,
            ...
            foreach($ticket_price_arr as $ticket){
                $ticketsTotal['sub_total'] += $ticket['sub_total'];
                $ticketsTotal['tax'] += $ticket['tax'];
                $ticketsTotal['total'] += $ticket['total'];
                $ticketsTotal['deposit'] += $ticket['deposit'];
            }
            
            return $ticketsTotal;
        }
        
        public static function getLocPairArr(array $location_arr){
            $length = count($location_arr);
            $location_pair = array();
            if(0 < $length){
                for($i=0; $i < $length; $i++) {
                    $j = $i+1;
